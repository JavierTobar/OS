...
int exec(char *p1, char *p2, char *p3) {
    ...
    if (file == NULL) {
        ...
        fileName = p1;
        if (launcher(file) != 1) {
            printf("Error launching the file %s \n", p1);
            return 1; // as soon as we have an error we just leave the function
        }
    }
    if (p2 != NULL) {
        file = fopen(p2, "r");
        if (file == NULL) {
            ...
            if (p3 != NULL) {
                ...
                if (file == NULL) {
                    ...
                    fileName = p3; // could potentially assign NULL to fileName once this segment is done
                    if (launcher(file) != 1) {
                        printf("Error launching the file %s \n", p3);
                        return 1; // as soon as we have an error we just leave the function
                    };
